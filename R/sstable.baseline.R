...
sstable.baseline <- function(
  ...
  fullfreq = FALSE,
  digits.numeric = c(
    "mean(x, na.rm = TRUE) > 10 ~ 0",
    "mean(x, na.rm=T) > 1 & mean(x, na.rm=T) <= 10 ~ 1",
    "mean(x, na.rm=T) <= 1 ~ 2"
  ),
  digits.nonnumeric = c("!is.numeric(x) ~ 0"),
  digits.name = NULL,
  test = FALSE,
  pdigits = 3,
  pcutoff = 0.0001,
  ...
  if (!is.null(y)) {
    ...
    if (is.factor(y)) {
      ...
      y <- factor(
        as.character(y),
        levels = sort(
          unique(as.character(y), decreasing = TRUE),
          na.last = TRUE
        ),
        exclude = NULL
      )
  ...
  if (!is.null(z)) {
    zdiscrete <- any(
      c("factor", "character", "logical") %in% class(unclass(z))
    ) |
      (any(c("numeric", "integer") %in% class(unclass(z))) &
        length(unique(na.omit(z))) <= 5)
  ...
  convert_to_function_list <- function(
    formula_list_numeric,
    formula_list_non_numeric,
    formula_list_name
  ) {
    ...
    convert_single <- function(fml_result) {
      ...
      formula_function <- eval(parse(
        text = paste0("function(x) { ", formula_str, " }")
      ))
    ...
    converted_list_non_numeric <- lapply(
      formula_list_non_numeric,
      convert_single
    )
    ...
    return(list(
      numeric = converted_list_numeric,
      non_numeric = converted_list_non_numeric,
      digits_name = converted_list_name
    ))
  }

  digits.check <- function(
    df,
    criteria_results_numeric,
    criteria_results_non_numeric,
  ...
  criteria <- convert_to_function_list(
    digits.numeric,
    digits.nonnumeric,
    digits.name
  )
  ...
  if (pooledGroup) {
    ...
    y <- factor(
      c(as.character(y), rep(ypool, nrow(dat))),
      levels = c(levels(y), ypool),
      exclude = NULL
    )
  ...
  value <- do.call(
    ...
    lapply(1:ncol(x), function(i) {
      sstable.baseline.each(
        ...
        z = z,
        bycol = bycol,
        pooledGroup = pooledGroup,
        statistics = statistics,
        continuous = continuous[i],
        fullfreq = fullfreq,
        test = test,
        digits = digits[i],
        pdigits = pdigits,
  ...
  value <- apply(value, 2, function(x) {
    ifelse(is.na(x) | x %in% c("NA (NA, NA)", "0/0 (NaN%)"), "-", x)
  })
  dim(value) <- value_dim
  ...
  if (matrix.raw) {
    ...
    tab <- sstable.baseline.edit(
      value = value,
      formula = formula,
      data = data,
      bycol = bycol,
      pooledGroup = pooledGroup,
      keepEmptyGroup = keepEmptyGroup,
      statistics = statistics,
      cont = cont,
      cate = cate,
      fullfreq = fullfreq,
      ...
      pdigits = pdigits,
      pcutoff = pcutoff,
      keepEmptyString = keepEmptyString,
      chisq.test = chisq.test,
      correct = correct,
      simulate.p.value = simulate.p.value,
      B = B,
...
sstable.baseline.each <- function(
  varname,
  label_list,
  x,
  y,
  ...
  mycont.summary <- function(x, y, z) {
    ...
    if (is.null(z)) {
      ...
      if (length(summarystat.nice) > 1) {
        result[1, seq(3, ncol(result), by = 2)] <- unlist(lapply(
          summarystat.nice,
      ...
      # Check if summarystat.nice has more than one value per row and flatten it if necessary
      if (length(summarystat.nice) > 1) {
        result[2:nrow(result), seq(3, ncol(result), by = 2)] <- unlist(lapply(
          summarystat.nice,
          function(x) paste(x, collapse = ", ")
        ))
      } else {
        result[2:nrow(result), seq(3, ncol(result), by = 2)] <- unlist(
          summarystat.nice
        )
      }
    }
    ...
    if (test == TRUE & ngroup > 1) {
      ...
      if (is.null(z)) {
        ...
        pval <- tryCatch(
          format.pval(
            kruskal.test(x = x[m], g = y[m])$p.value,
            eps = pcutoff,
            digits = pdigits,
            scientific = FALSE
          ),
        ...
        pval <- sapply(1:length(levels(x)), function(i) {
          ...
          if (length(q) == 0) {
            ...
            out <- tryCatch(
              format.pval(
                kruskal.test(x = z[m], g = y[m])$p.value,
                eps = pcutoff,
                digits = pdigits,
                scientific = FALSE
              ),
  ...
  mycat.summary <- function(x, y, z, fullfreq = fullfreq) {
    ...
    if (is.null(z)) {
      ...
      ta.nice <- matrix(
        paste0(
          " (",
          formatC(100 * unclass(ta.prop), digits, format = "f"),
          "%)"
        ),
        nrow = nrow(ta),
        ncol = ncol(ta)
...
sstable.baseline.edit <- function(
  value,
  formula,
  data,
  bycol = TRUE,
  ...
  if (length(digits) == 1) {
    ...
    if (length(digits) != ncol(x)) {
      stop(
        "digits argument must have length 1 or similar length as number of row-wise variables !!!"
      )
    }
  }
  ...
  if (!keepEmptyString) {
    value <- value %>% filter(V1 != "- ")
  } #remove empty string -hungtt
  ...
  header1 <- c(
    ...
    c(rbind(
      rep("", length(gr.lev)),
      paste(gr.lev, " (N=", table(y), ")", sep = "")
    ))
  )
  header2 <- c(
    "Characteristic",
    rep(c("n", "Summary statistic"), length(gr.lev))
  )
  if (test) {
    header1 <- c(header1, "p value")
    header2 <- c(header2, "")
  ...
  if (flextable) {
    ...
    ## header
    header1[1] <- header2[1]
    header1[seq(from = 2, to = 2 * length(gr.lev), by = 2)] <- header1[
      seq(from = 2, to = 2 * length(gr.lev), by = 2) + 1
    ]
    if (test) {
      header2[length(header1)] <- header1[length(header1)]
    }
    assign(
      "tab",
      eval(parse(
    ...
    tab <- list(table = rbind(header1, header2, value), footer = footer)
    class(tab$table) <- c('baseline_tbl', 'ss_tbl', class(tab$table))
  }
  ## output
  if (!flextable) {
